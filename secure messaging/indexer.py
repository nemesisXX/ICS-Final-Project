...
class Index:
    ...
    def indexing(self, m, l):
        ...
        for word in words:
            self.index[word.lower().translate(str.maketrans("", "", string.punctuation))] = \
                self.index.get(word.lower().translate(str.maketrans("", "", string.punctuation)), []) + [l]
        # ---- end of your code --- #
        return
    ...
    def search(self, terms):
        ...
        # ---- start your code ---- #
        try:
            for i in self.index[term[0].lower().translate(str.maketrans("", "", string.punctuation))]:
                if terms.lower().translate(str.maketrans("", "", string.punctuation)) in \
                        self.msgs[i].lower().translate(str.maketrans("", "", string.punctuation)):
...
class PIndex(Index):
    ...
    def load_poems(self):
        ...
        for line in poems.readlines():
            if line.rstrip()[:-1] not in self.int2roman.values():
                self.add_msg_and_index(line.strip())
            else:
                self.add_msg_and_index("Chapter " + line.strip())
    ...
    def get_poem(self, p):
        ...
        # IMPLEMENTATION
        # ---- start your code ---- #
        roman = "Chapter " + self.int2roman[p]
        roman += "."
        start = self.search(roman)[0][0]
        try:
            end = self.search("Chapter " + self.int2roman[p+1] + ".")[0][0]
        except KeyError:
            end = self.total_msgs
